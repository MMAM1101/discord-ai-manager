...
client.on('messageCreate', async (message: Message) => {
  ...
  if (botConfig.aiChannelId && message.channelId === botConfig.aiChannelId) {
    ...
    try {
      const aiChannelSystemPrompt = `أنت مساعد الإشراف الذكي والمبرمج للبوت في خادم ديسكورد.
مهمتك هي قراءة الرسالة الواردة من المشرف وتحديد إذا كانت طلباً لتنفيذ إجراء إداري حقيقي لحفظ وتعديل الخادم (مثل: إنشاء رتبة/رول، إنشاء روم/قناة صوتية أو كتابية، أو تصميم تصويت/استبيان، أو برمجة ردود تلقائية تتابعية أو عادية).

عليك الرد دائماً بكود JSON نظيف مطابق تماماً للنموذج المقترح:
{
  "isAction": true أو false (حدد true فقط إذا كان هناك طلب إداري أو أمر برمجي مباشر يريد تنفيذ شيء مثل سوي رتبة، سوي قناة، ارسل تصويت، برمج كذا، سوي كذا)،
  "response": "ردك الودي والمتحمس والمختصر جداً باللغة العربية مع الإشارة لتنفيذ الطلب أو الإجابة التبادلية الودية للدردشة والمجاملة وبسطور متباعدة ومظهر جذاب ومناسب لديسكورد ومؤثرات الخطوط والسطور التوضيحية لتقديم لمسة جمالية",
  "rule": { // يتوفر فقط إذا كان isAction هو true
    "name": "اسم مميز ومختصر باللغة العربية للعملية (مثل: إنشاء رتبة الإدارة)",
    "trigger": "الكلمة المفتاحية أو جملة تشغيل الأمر (مثلاً: 'vip' أو 'رتبة ادمن')",
    "matchType": "exact" أو "flexible",
    "actionType": "create_role" | "create_channel" | "poll" | "reply_text" | "reply_image" | "sequential_replies",
    "explanation": "شرح مفصل وودي باللغة العربية لما يتم تفعيله وتعديله فوراً في النظام لخدمة طلبك",
    "actionData": {
       // املأ البيانات الملائمة لنوع الـ actionType المحدد:
       // - لـ "reply_text": { "text": "نص الرد التلقائي" }
       // - لـ "reply_image": { "imageUrl": "رابط الصورة العشوائية من unsplash" }
       // - لـ "sequential_replies": { "replies": [ { "type": "image"|"text", "content": "الرابط أو النص" } ] }
       // - لـ "create_role": { "roleName": "اسم الرتبة المطلوبة", "color": "رمز لون هيكس HEX مثل #ef4444 للون الأحمر", "permissions": ["Administrator"] }
       // - لـ "create_channel": { "channelName": "اسم القناة بصيغة كباب كيس", "type": "text"|"voice" }
       // - لـ "poll": { "title": "عنوان الاستبيان المطلوب التفاعل عليه" }
    }
  }
}`;
      ...
      const response = await ai.models.generateContent({
        ...
        contents: content,
        config: {
          systemInstruction: aiChannelSystemPrompt,
          responseMimeType: "application/json",
          responseSchema: {
            type: Type.OBJECT,
            required: ["isAction", "response"],
            properties: {
      ...
      if (!jsonText) throw new Error("لم يتمكن الذكاء الاصطناعي من تحليل طلبك بشكل مهيكل.");

      const parsed = JSON.parse(jsonText);
      const replyText = parsed.response || "تم استلام طلبك وجاري معالجته.";

      // 1. إرسال الرد النصي الأساسي أولاً
      ...
      // 2. إذا تضمن طلباً إدارياً أو حدثاً مبرمجاً، قم بتنفيذه وحفظه فوراً في ديسكورد!
      if (parsed.isAction && parsed.rule) {
        const pRule = parsed.rule;
        const newRule: CustomRule = {
          id: `rule-${Date.now()}`,
          name: pRule.name,
          trigger: pRule.trigger,
          matchType: pRule.matchType || 'exact',
          actionType: pRule.actionType,
          explanation: pRule.explanation,
          actionData: pRule.actionData,
          createdBy: message.author.tag,
          createdAt: new Date().toISOString()
        };

        // حفظ في القائمة الحية
        ...
        try {
          if (newRule.actionType === 'create_role') {
            if (!message.guild?.members.me?.permissions.has(PermissionsBitField.Flags.ManageRoles)) {
              detailsMessage = "⚠️ تم حفظ وبرمجة الأمر، ولكن تعذر إنشاؤها فوراً لعدم امتلاك البوت صلاحية `إدارة الأدوار` (Manage Roles) في السيرفر حالياً.";
            } else {
              const rData = newRule.actionData;
              const role = await message.guild.roles.create({
                ...
                reason: 'تم الإنشاء تلقائياً ومزامنة الذكاء الاصطناعي فوراً كطلب مباشر'
              });
              detailsMessage = `✅ **تم إنشاء الرتبة بنجاح واستقرت بالخادم:** ${role}`;
            }
          } else if (newRule.actionType === 'create_channel') {
            if (!message.guild?.members.me?.permissions.has(PermissionsBitField.Flags.ManageChannels)) {
              detailsMessage = "⚠️ تم حفظ وبرمجة الأمر، ولكن تعذر إنشاؤه فوراً لعدم امتلاك البوت صلاحية `إدارة القنوات` (Manage Channels).";
            } else {
              const cData = newRule.actionData;
              const channelType = cData.type === 'voice' ? ChannelType.GuildVoice : ChannelType.GuildText;
              const newChannel = await message.guild.channels.create({
                name: cData.channelName || "روم-جديد",
                type: channelType,
                reason: 'تم إنشاء الروم فوراً بطلب ذكاء اصطناعي'
              });
              detailsMessage = `✅ **تم إنشاء القناة الجديدة بنجاح في تصنيفات سيرفرك:** ${newChannel}`;
            }
          } else if (newRule.actionType === 'poll') {
            const pData = newRule.actionData;
            const pollEmbed = new EmbedBuilder()
              .setColor('#ff9100')
              .setTitle(`Vote: ${pData.title || 'تصويت إداري فوري'}`)
              .setDescription('تفاعل بالتصويت باستخدام الرموز التعبيرية المفضلة لأسفل لإبداء الرأي!')
            ...
            await pollMsg.react('👎');
            await pollMsg.react('🤷');
            detailsMessage = `✅ **تم إطلاق ونشر التصويت الإداري فوراً للأعضاء بنجاح!**`;
          } else {
            // للرد التلقائي والنصوص التتابعية
            detailsMessage = `✅ تم برمجة وتفعيل هذا الأمر الجديد بنجاح في السيرفر! مفعّل الأمر هو: \`${newRule.trigger}\``;
          }

          // إرسال كرت إيجاز النجاح والتبشير المشرف
          const embedColor = newRule.actionType === "create_role" ? "#3b82f6" : 
                             newRule.actionType === "create_channel" ? "#10b981" : 
                             newRule.actionType === "poll" ? "#f59e0b" : "#ab47bc";

          const successEmbed = new EmbedBuilder()
            .setColor(embedColor)
            .setTitle(`⚡ تم تطبيق الإجراء الإداري فوراً بالبرمجة الذاتية!`)
            .setDescription(detailsMessage)
            .addFields(
              { name: '💎 اسم الميزة/الأمر', value: `\`${newRule.name}\``, inline: true },
              { name: '🎯 الكلمة المفعلة (Trigger)', value: `\`${newRule.trigger}\``, inline: true },
              { name: '🛠️ نوع العملية', value: `\`${newRule.actionType}\``, inline: true },
              { name: '📝 التوضيح الفلسفي للميزة', value: newRule.explanation }
            )
            .setFooter({ text: 'تم تصميم الشيفرة البرمجية بالكامل وحفظها وقصها حياً عبر Gemini Core' });

          await (message.channel as any).send({ embeds: [successEmbed] });

        } catch (execErr: any) {
          console.error("Direct Action Execution Error:", execErr);
          await message.reply(`❌ تم حفظ الميزة بنجاح ولكن حصل خطأ فني أثناء التنفيذ المباشر: ${execErr.message}`);
        }
      }

    } catch (gemIniErr: any) {
      console.error("AI Room Response Error:", gemIniErr);
      await message.reply(`❌ عذراً، حصل خطأ أثناء التفكير وتوليد الاستجابة الإدارية: ${gemIniErr.message}`);
    }
...
أرجع كائن JSON موافق تماماً للنموذج التالي ليتم حفظه بملفات البوت وتفعيله فوراً بلا توقف:
{
  ...
  "actionData": {
     ...
     // - لـ "poll": { "title": "استفتاء الإدارة", "options": ["خيار أ", "خيار ب"] }
      ...
      let directExecutionFeedback = "تم تسجيل الأمر ونشره في مستمعي البوت النشطين. يمكنك تفعيله بكتابة الكلمة المفتاحية!";
      
      try {
        if (newRule.actionType === 'create_role') {
          if (!message.guild?.members.me?.permissions.has(PermissionsBitField.Flags.ManageRoles)) {
            directExecutionFeedback = "⚠️ تم برمجة الميزة، ولكن لا تتوفر لدي صلاحية `إدارة الأدوار` (Manage Roles) لإنشاء الرتبة حالياً في السيرفر.";
          } else {
            const rData = newRule.actionData;
            const role = await message.guild.roles.create({
              name: rData.roleName || "رتبة جديدة مبرمجة",
              color: rData.color || "#00f0ff",
              permissions: rData.permissions?.includes("Administrator") ? [PermissionsBitField.Flags.Administrator] : [],
              reason: 'تم الإنشاء فوراً بطلب المشرف عبر أمر البرمجة المباشر'
            });
            directExecutionFeedback = `✅ **تم إنشاء وتفعيل الرتبة المطلوبة فوراً وبنعومة بالخادم!**\nاسم الرتبة: ${role}`;
          }
        } else if (newRule.actionType === 'create_channel') {
          if (!message.guild?.members.me?.permissions.has(PermissionsBitField.Flags.ManageChannels)) {
            directExecutionFeedback = "⚠️ تم برمجة الميزة، ولكن لا تتوفر لدي صلاحية `إدارة القنوات` (Manage Channels) لإنشاء القناة.";
          } else {
            const cData = newRule.actionData;
            const channelType = cData.type === 'voice' ? ChannelType.GuildVoice : ChannelType.GuildText;
            const newChannel = await message.guild.channels.create({
              name: cData.channelName || "روم-جديد",
              type: channelType,
              reason: 'تم إنشاء القناة فوراً بطلب المشرف عبر أمر البرمجة المباشر'
            });
            directExecutionFeedback = `✅ **تم إنشاء وتثبيت القناة المطلوبة فوراً بطلبك بالخادم!**\nالروم: ${newChannel}`;
          }
        } else if (newRule.actionType === 'poll') {
          const pData = newRule.actionData;
          const pollEmbed = new EmbedBuilder()
          ...
          await pollMsg.react('👎');
          await pollMsg.react('🤷');
          directExecutionFeedback = "✅ **تم إطلاق التصويت الإداري بنجاح وتوجيهه للمجتمع فوراً!**";
        }
      } catch (exErr: any) {
        console.error("Direct execution in !برمج error:", exErr);
        directExecutionFeedback = `⚠️ تم تفعيل برمجة الميزة، ولكن حصل خطأ فني أثناء محاولة التنفيذ الفوري لها بالخلفية: ${exErr.message}`;
      }

      const successEmbed = new EmbedBuilder()
        .setColor('#2e7d32')
        .setTitle('✅ تم البرمجة والتركيب الذاتي بنجاح!')
        .setDescription(`مرحباً **${message.author.username}**، قام محرك الذكاء الاصطناعي ببناء الكود ونشره في عمليات البوت الحية حالياً وهو مستعد للخدمة الفورية!\n\n**حالة التنفيذ الفوري لعملية الحفظ:**\n${directExecutionFeedback}`)
        .addFields(
